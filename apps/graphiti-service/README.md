...
Python/FastAPI sidecar that provides the knowledge graph brain for laura-core. Wraps [Graphiti](https://github.com/getzep/graphiti) with a [Kuzu](https://kuzudb.com/) embedded graph database — no separate DB server required.
...
- Called by laura-core after each memory flush (episode ingestion) and before each agent turn (context injection)
...
|---|---|---|
| `KUZU_DB` | `../laura-core/context/memory/graph.kuzu` | Path to the Kuzu graph database directory |
...
| `GROUP_ID` | `laura` | Scopes all episodes to a single user namespace |
...
```json
{
  ...
  "source_description": "laura conversation"
}
```
